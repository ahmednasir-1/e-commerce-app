...
export default function Signup() {
  ...
  const sendEmailOtp = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/send-email-otp', { email: form.email, name: form.name });
      toast.success('Verification code sent to your email');
      setStep(2);
    } catch {} finally { setLoading(false); }
  };

  // Step 2 — verify email OTP then create account directly
  const verifyAndCreate = async (e) => {
    ...
    try {
      ...
      // Create account immediately — phone saved but not OTP verified
      ...
      <div className="w-full max-w-sm bg-white rounded-2xl border border-amber-100 p-8 shadow-sm">
        ...
        {step === 1 && (
          <form onSubmit={sendEmailOtp} className="space-y-4">
            ...
            {/* Phone — collected but not OTP verified */}
            <div>
              <label className={labelCls}>
                Phone Number
                <span className="ml-2 normal-case text-stone-300 font-normal tracking-normal">
                  (for order SMS updates)
                </span>
              </label>
        ...
        {step === 2 && (
          <form onSubmit={verifyAndCreate} className="space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 text-sm text-stone-500">
              ...
              <span className="text-amber-600 font-semibold">{form.email}</span>.
              Check your inbox.
            </div>
            ...
            <button
              ...
              className="w-full text-xs text-amber-600 font-medium hover:underline bg-transparent border-none cursor-pointer disabled:opacity-40">
            ...
            <button
              type="button"
              onClick={() => setStep(1)}
              className="w-full text-xs text-stone-400 font-medium hover:underline bg-transparent border-none cursor-pointer">
              ← Go back
            </button>
          </form>
        )}
